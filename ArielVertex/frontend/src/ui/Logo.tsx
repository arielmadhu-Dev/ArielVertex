...
export function Logo({ compact = false, inverse = false, className }: { compact?: boolean; inverse?: boolean; className?: string }) {
  return (
    <div className={cx('flex items-center gap-2.5 select-none', className)}>
      <VertexMark size={30} className={cx(inverse ? 'text-white' : 'text-navy dark:text-white', 'shrink-0')} />
      {!compact && (
        <div className="leading-none">
          <div className="text-[17px] font-extrabold tracking-tight">
            <span className={inverse ? 'text-white' : 'text-navy dark:text-white'}>Ariel</span>{' '}
            <span className={inverse ? 'text-brand-400' : 'text-brand-500'}>Vertex</span>
          </div>
          <div className={cx('mt-0.5 text-[9.5px] font-medium uppercase tracking-[0.14em]', inverse ? 'text-[#74829e]' : 'text-slate-400')}>
